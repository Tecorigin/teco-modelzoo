...
本文档介绍如何检查Tecorigin ModelZoo的开发环境，包括硬件环境检查和容器环境检查。
...
为便于您能够快速使用Tecorigin ModelZoo执行模型训练或推理等任务，Tecorigin ModelZoo当前以Docker容器的方式提供服务，Docker容器已经包含使用所需的所有基础软件及深度学习框架TecoPyTorch 和TecoPaddle。在进入Docker后，您可以按照以下步骤查看容器中的相关软件信息。

1. 在容器中，执行以下命令，查看Conda基础环境信息。
   请以实际使用环境为准（如国产机器无conda环境，直接执行python -c "import torch,torch_sdaa"即可验证环境组件）。
   ```
   ...
   ```
3.  进入Conda环境，执行以下命令，查看框架及其依赖组件信息。以TecoPyTorch为例：
    ```
...
4. 若环境为国产环境，部分不存在conda配置，请根据实际情况，检查TecoPytorch及其依赖组件的版本是否正常。
