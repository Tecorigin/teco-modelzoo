...
class EntryPoint:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if(self.model.arch == "fused_resnet50"):
            try:
                from .resnet import ResNet,Bottleneck
                unfused_constructor = ResNet
                
                unfused_arch = ResNet.Arch(
                        stem_width=64,
                        block=Bottleneck,
                        layers=[3, 4, 6, 3],
                ...
                unfused_resnet50 = unfused_constructor(arch=unfused_arch, **asdict(params))
                model = self.model.constructor()
                model.load_state_model(unfused_resnet50)
            except:
                print('load fused resnet failed ,use torchvision instead')
                from torchvision import models
                unfused_resnet50 = models.resnet50()
                model = self.model.constructor()
                model.load_state_model(unfused_resnet50)
        else:
        ...
        state_dict = None
        if pretrained:
            if self.name == "fused_resnet50":
                from torchvision import models
                unfused_resnet50 = models.resnet50(pretrained=True)
                model.load_state_model(unfused_resnet50)
                state_dict = None
            else:
        ...
        if pretrained_from_file is not None:
            if os.path.isfile(pretrained_from_file):
                ...
                state_dict = torch.load(
                    pretrained_from_file, map_location=torch.device("cpu"), weights_only=False
                )
            else:
                print(
